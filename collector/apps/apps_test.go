...
func TestLoadValid(t *testing.T) {
	...
	// view / parser 힌트는 해석하지 않고 원문 그대로 보존되어야 한다
	...
	if !strings.Contains(string(fwd.Parser), `"event_time"`) {
		t.Errorf("parser 힌트가 원문 그대로 보존되지 않았다: %s", fwd.Parser)
	}
	if stt.Parser != nil {
		t.Errorf("parser 가 없는 앱에 값이 생겼다: %s", stt.Parser)
	}
